...
// handleNewRecording saves new recorded audio snippet to a fileformat.WriteWavFile file.
func handleNewRecording(socket socketio.Conn, recordData string) {
	...
	now := time.Now()
	fileName := fmt.Sprintf("%04d_%02d_%02d_%02d_%02d_%02d.fileformat.WriteWavFile",
	...
	err = fileformat.WriteWavFile(filePath, decodedAudioData, recData.SampleRate, recData.Channels, recData.SampleSize)
	if err != nil {
		err := xerrors.New(err)
		logger.ErrorContext(ctx, "Failed write fileformat.WriteWavFile file.", slog.Any("error", err))
	}
}
...
func handleNewFingerprint(socket socketio.Conn, fingerprintData string) {
	...
	var data struct {
		Fingerprint map[int64]uint32 `json:"fingerprint"`
	}
	...
	matches, _, err := core.FindMatchesUsingFingerPrints(data.Fingerprint)
	if err != nil {
		err := xerrors.New(err)
